...
to inicializar
  ...
  ifelse tipo_habitat = "piedra volcánica" [ set color_camuflaje black ][ set color_camuflaje brown ]
...
to colorear_celdas
  let color_arena brown + 2
  let color_piedra_volcanica black + 2
  ask celdas_reproduccion [ set pcolor gray + 3 ]
  if tipo_habitat = "piedra volcánica" [
    ask celdas_generacion_1 [ set pcolor color_piedra_volcanica ]
    ask celdas_generacion_2 [ set pcolor color_piedra_volcanica ]
  ]
  if tipo_habitat = "arena" [
    ask celdas_generacion_1 [ set pcolor color_arena ]
    ask celdas_generacion_2 [ set pcolor color_arena ]
  ]
  ask etiquetas [ die ]
end
...
to ejecutar_generacion
  if count ratones-on celdas_generacion_2 >= tamanio_pob [
    ejecutar_acciones_nueva_generacion
    seleccion_natural
  ]
...
to seleccion_natural
  ...
  create-halcones 3 [
    ...
    set size 2
    set color brown + 4
    move-to one-of celdas_generacion_1
  ]
  repeat 100 [
    ask halcones [
      move-to one-of celdas_generacion_1 in-radius 1
      let raton_camuflaje one-of ratones-here with [color = color_camuflaje]
      let raton_no_camuflaje one-of ratones-here with [color != color_camuflaje ]
      if raton_camuflaje != nobody and random-float 1 < prob_mort_camuflaje [
        ask raton_camuflaje [ die ]
      ]
      if raton_no_camuflaje != nobody and random-float 1 < prob_mort_no_camuflaje [
        ask raton_no_camuflaje [ die ]
      ]
      display
    ]
  ]
...
615
N
count ratones-on celdas_generacion_1
0
1
11
